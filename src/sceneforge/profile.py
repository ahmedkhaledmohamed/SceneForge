...
@dataclass
class Seed:
    """A piece of global context: an inspiration image, a reference clip,
    or a text note. Library material for browsing and brainstorming."""
    id: str
...
class Profile:
    ...
    def add_character(self, name: str, description: str = "",
        ...
        if not self.characters:
            character.main = True  # first character is the default
...
def find_profile_root(start: Path) -> Path | None:
    """Walk up from a project dir looking for profile.json (the project
    lives under <profile>/projects/<slug>)."""
...
def resolve_character(project: Project, profile: Profile | None,
    ...
    """The context chain: pchar-* resolves against the profile (refs are
    profile-relative), char-* against the project. Returns (character,
    base dir its reference_images are relative to)."""
    if character_id.startswith("pchar-"):
        if profile is None:
            raise KeyError(
                f"Scene references profile character '{character_id}' but the "
                "project is not inside a profile (expected <profile>/projects/...)"
            )
